import { Kafka, Producer as KafkaProducer, Message as KafkaMessage, RecordMetadata, CompressionTypes } from 'kafkajs'
import {Writable} from 'stream'
import { ProducerParams } from './types';

export class Producer extends Writable {
  private producer: KafkaProducer
  constructor(private kafka: Kafka, private params: ProducerParams){
  ...
  _write(chunk: KafkaMessage, _: BufferEncoding, callback: (error?: Error) => void): void {
    ...
    if(this.chunks.length > this.params.bufferMessages || this.lastSend + this.params.bufferTime_ms > Date.now()){
      const chunks = [...this.chunks];
      this.chunks = [];
      this.send(this.params.topic, chunks).then(() => callback()).catch(callback);
    } else {
      callback();
    }
  }

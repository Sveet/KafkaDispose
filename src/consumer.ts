...
export class Consumer extends Readable {
  ...
  private async run() {
    ...
    this.consumer.run({
      ...
      eachBatch: async ({batch, isRunning, isStale, resolveOffset, heartbeat, commitOffsetsIfNecessary}) => {
        using _ = new HeartbeatManager(heartbeat);
        const completePromises = []
        for(const message of batch.messages){
          let completeResolver: () => void;
          completePromises.push(new Promise<void>(resolve => completeResolver = resolve))
